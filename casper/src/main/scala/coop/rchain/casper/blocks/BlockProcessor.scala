...
class BlockProcessor[F[_]: Concurrent](
  ...
  def checkIfOfInterest(c: Casper[F], b: BlockMessage)(
    ...
    val shardOfInterest   = c.getGenesis.map(_.shardId.equalsIgnoreCase(b.shardId))
    val versionOfInterest = c.getGenesis.flatMap(g => Validate.version(b, g.header.version))
    val oldBlock =
      c.getGenesis.flatMap(g => (ProtoUtil.blockNumber(b) < ProtoUtil.blockNumber(g)).pure[F])
