...
//            case Left(e) => {
//              println(e)
//              (ByteString.EMPTY, Seq.empty, Seq.empty)
//            }
//            case Right(v) => v
//          }
    }.toList
...
//          baseChildren1 <- mkAllStates(baseChildren.head._1, 3, 3)
  ...
  "what" should "the heck" in effectTest {
    runtimeManagerResource.use { runtimeManager =>
      {
        implicit val rm: RuntimeManager[Task] = runtimeManager
        val genesisPostStateHash              = genesis.body.state.postStateHash
        ...
        for {
          // create base state - child of genesis
          b  <- mkSingleState(genesisPostStateHash, 1, 1)
          b1 <- mkSingleState(b._1, 2, 2)
        } yield ()
      }
    }
  }

  "what" should "the heck 2" in effectTest {
    runtimeManagerResource.use { runtimeManager =>
      {
        val genesisPostStateHash = genesis.body.state.postStateHash
        ...
        for {
          // create base state - child of genesis
          b1 <- makeTxAndCommitState(
                 runtimeManager,
                 genesisPostStateHash,
                 genesisContext.genesisVaults.head._1,
                 genesisContext.validatorPks.head,
                 1,
                 1
               )
          _ <- makeTxAndCommitState(
                runtimeManager,
                b1._1,
                genesisContext.genesisVaults.head._1,
                genesisContext.validatorPks.head,
