...
trait MergeabilityRules {
  case class State(
    ...
    def ++(obj: Any): State = obj match {
      case State(dataOther, continuationOther, joinOther) =>
        State(
          dataOther ++ datas,
          continuationOther ++ continuations,
          // joins are not duplicate
          (joins.toSet ++ joinOther.toSet).toSeq
        )
      case _ => throw new Exception(s"Unsupported type ${obj}")
    }
    ...
    override def equals(obj: Any): Boolean = obj match {
      case State(dataOther, continuationOther, joinOther) =>
        datas.length == dataOther.length &&
          continuations.length == continuationOther.length &&
          datas.forall(
            d =>
          ...
          ) && joins.toSet == joinOther.toSet
      case _ => false
    }
  }
  val emptyState = State(Seq.empty, Seq.empty, Seq.empty)
  ...
  case class Rho(
    ...
    lazy val rstate        = state.runSyncUnsafe()
...
//    "(4!!) C! 2"   -> NonDeterminedMergeableCase(R1, F_)(C_)(S0)(emptyState)(emptyState),
  ...
  val peekMergeabilityCases = List(
    "PX !X"    -> ConflictingCase(S0)(P_)(Nil)(S0.rstate)(P_.rstate),
...
//    "P! (!4)"   -> NonDeterminedUnknownCase(S0, F0)(S1)(P_)(emptyState)(emptyState),
    ...
    "!P !4 2"   -> ConflictingCase(F_)(P1)(S1)(emptyState)(S1.rstate),
    "(!P) !4"   -> CoveredBy("(P!) !4"),
    "!P (!4)"   -> MergeableCase(F0)(P1)(S0, S1)(S1.rstate),
    "(!P) (!4)" -> CoveredBy("(P!) (!4)"),
